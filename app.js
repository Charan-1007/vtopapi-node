...
const checkForErrors = async (page) => {
    try {
        ...
        if (bodyText.includes("Invalid Captcha")) {
            return "captcha";
        } else if (bodyText.includes("Invalid LoginId/Password")) {
            return "login";
        } else if (bodyText.includes("Invalid credentials.")) {
            return "credentials";
        }
        return null;
    } catch (e) {
...
const getVtopData = async (username, password, semIndex = 0) => {
    ...
    const context = await browser.newContext({ userAgent: USER_AGENT_STRINGS[Math.floor(Math.random() * USER_AGENT_STRINGS.length)] });
    const page = await context.newPage();

    try {
        await page.goto("https://vtop.vit.ac.in/vtop", { waitUntil: "domcontentloaded" });
        await page.waitForSelector("#stdForm", { state: "visible", timeout: 50000 });
        ...
        await page.waitForSelector(":has-text('VTOP Login')", { state: "visible", timeout: 50000 });
        console.log("Login page fully loaded");

        while (true) {
            try {
                await page.waitForSelector("#captchaStr", { state: "visible", timeout: 10000 });
                console.log("Captcha found. Proceeding with login.");
                break;
            } catch (e) {
                console.log("Captcha not detected, reloading page quickly...");
                await page.reload();
            }
        }
